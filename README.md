...
The initial conditions of the MCMC fundamentally are the economic rules (`eco_rules`) and the gamestate. I define the gamestate as each teams' money, loss counter and the score. The economic rules contains what I call the `market` (the price of all purchasable goods), as well as predefined values such as round win/loss rewards, starting money, max money, max loss, etc. Also in the economic rules are values calculated based on the market/my intuitions regarding the game/statistical analysis which impact the liklihood of different events occuring in a round. This is a main area of investigation as some values are missing validation/theory. Technically the chain is also a function of the buying strategy, and thus the buying strategy is also an initial condition. Currently, this process is hard-coded into the simulation. This is another area of investigation. 

In `CS_ntbk` there is an implementation of the module. First you will see the MCMC initial conditions defined and following will be a simulation of a single half with a printout of the gamestate after every round (this is shown in the table below). Below that you will see a simulation of size $n$ stored and used for statistical analysis/visualization. Note that in the notebook there is only a call to `half_simulator()`, which is simply a wrapper for the `play_round()` function, which is where most of the actual round simulation mechanics are called. I talk about this more deeply in **Methodology**. 

If we look at the below table, those experienced in CS will notice some oddities that I would like to clarify. Firstly, it can be seen that what I am calling a team's money is around what a single player on that team would have, indicating I am using the average of each players money on team as a proxy for team money. In CSGO, money is not literally shared between players, but through dropping weapons money essentially becomes shared across a team over time. Thus, I find it sufficient to only track a single players economy for this level of simulation. This is a limitation, but not one that I see to have to many impliciations (at least not currently). Another limitiations is in the buying strategies. Firstly, there are no double-ecos (a team saving on two consecutive rounds). Second, there are no "eco-buys", or teams buying into a round just enough that they can also buy the next round. Saving to buy next round implies saving all of your money in the current implementation. This is also a limitiation that I do plan to address. Finally we see that teams are considered to be "forcing" on the first round of the game. Because as a team you can't afford a full-buy in the first round, but neither can your opponent, my buy logic calls for a force, and the `force_percent` described earlier is defined as 

$$ \text{force_percent} = \frac{\text{Cost of Vest}}{\text{Starting Money}} $$ 

which is exactly in line with this definition. If there are ideas of tweaking the starting money, this value might come into question. Strategies like saving first round or spending much less are ideas I think should be evaluated, but currently are not an option for a team to take. 


**Insert Table as example of simulated half**
